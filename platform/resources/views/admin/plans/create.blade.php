<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                <nav class="flex items-center gap-2 text-sm font-bold text-gray-400 mb-2">
                    ...
                    <span aria-hidden="true" class="material-symbols-outlined text-[16px]">chevron_right</span>
                    <span class="text-primary">Criar Novo Plano</span>
                </nav>
                <h1 class="text-4xl font-extrabold text-duo-heading dark:text-white tracking-tight">Configurar Plano</h1>
                <p class="text-gray-400 mt-1">Defina os preços, regras e limites do seu plano de assinatura.</p>
            </div>
    ...
    <form id="plan-form" action="{{ route('admin.plans.store') }}" method="POST">
        @csrf
        ...
        <div class="bg-white rounded-2xl border-2 border-duo-border overflow-hidden mb-6">
            ...
            <div class="p-6 space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Nome do Plano *</label>
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="Ex: Premium Gold"
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Público-Alvo *</label>
                        <select name="target_audience" required
                            aria-label="Público-alvo"
                            class="w-full px-4 py-3 bg-white border-2 border-duo-border rounded-xl text-sm font-medium focus:border-primary focus:ring-0 transition-colors">
                            <option value="both" {{ old('target_audience') == 'both' ? 'selected' : '' }}>Professor e Instituição</option>
                            <option value="teacher" {{ old('target_audience') == 'teacher' ? 'selected' : '' }}>Apenas Professor</option>
                            <option value="institution" {{ old('target_audience') == 'institution' ? 'selected' : '' }}>Apenas Instituição</option>
                        </select>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Preço Original (R$) *</label>
                        <input type="number" name="original_price" value="{{ old('original_price', '0.00') }}" step="0.01" min="0" required
                            aria-label="Preço original em reais"
                            class="w-full px-4 py-3 bg-white border-2 border-duo-border rounded-xl text-sm font-medium focus:border-primary focus:ring-0 transition-colors">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Ordenação</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0"
                            aria-label="Ordenação"
                            class="w-full px-4 py-3 bg-white border-2 border-duo-border rounded-xl text-sm font-medium focus:border-primary focus:ring-0 transition-colors">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Nível/Tier</label>
                        <input type="number" name="tier_level" value="{{ old('tier_level', 0) }}" min="0"
                ...
                <div class="bg-background-light border-2 border-duo-border rounded-xl p-5">
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="flex items-center justify-between cursor-pointer">
                            ...
                            <input type="checkbox" name="is_most_popular" value="1" {{ old('is_most_popular') ? 'checked' : '' }}
                                class="w-10 h-5 rounded-full bg-gray-200 checked:bg-primary border-0 cursor-pointer transition-colors">
                        </label>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl border-2 border-duo-border overflow-hidden mb-6">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Preço Promocional (R$)</label>
                        <input type="number" name="promotional_price" value="{{ old('promotional_price') }}" step="0.01" min="0" placeholder="Deixe vazio se não houver"
                            aria-label="Preço promocional em reais"
                            class="w-full px-4 py-3 bg-white border-2 border-duo-border rounded-xl text-sm font-medium focus:border-primary focus:ring-0 transition-colors">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Início da Promoção</label>
                        <input type="datetime-local" name="promo_starts_at" value="{{ old('promo_starts_at') }}"
                            aria-label="Início da promoção"
                            class="w-full px-4 py-3 bg-white border-2 border-duo-border rounded-xl text-sm font-medium focus:border-primary focus:ring-0 transition-colors">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Fim da Promoção</label>
                        <input type="datetime-local" name="promo_ends_at" value="{{ old('promo_ends_at') }}"
        ...
        <div class="bg-white rounded-2xl border-2 border-duo-border overflow-hidden mb-6">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="flex items-center justify-between bg-background-light border-2 border-duo-border rounded-xl px-5 py-4">
                    <div class="flex items-center gap-3">
                    ...
                    <input type="number" name="limits[{{ $key }}]" value="{{ old("limits.$key") }}" min="0" placeholder="∞"
        ...
        <div class="bg-white rounded-2xl border-2 border-duo-border overflow-hidden mb-8">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <label class="flex items-center gap-3 bg-background-light border-2 border-duo-border rounded-xl px-5 py-4 cursor-pointer hover:border-primary transition-colors">
                        <input type="hidden" name="features[{{ $key }}]" value="0">
                        <input type="checkbox" name="features[{{ $key }}]" value="1" {{ old("features.$key") ? 'checked' : '' }}
