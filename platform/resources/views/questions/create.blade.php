            ...
            <div>
                <nav class="flex items-center gap-2 text-sm font-bold text-gray-400 mb-2">
                    ...
                    <a class="hover:text-primary transition-colors" href="{{ route('questions.index') }}">Banco de
                        Questões</a>
                    <span aria-hidden="true" class="material-symbols-outlined text-[16px]">chevron_right</span>
                    <span class="text-primary">Nova Questão</span>
                </nav>
                <h1 class="text-4xl font-extrabold text-duo-heading dark:text-white tracking-tight">Nova Questão</h1>
            </div>
            ...
            <form action="{{ route('questions.store') }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="type" name="type"
                            ...
                            <option value="true_false" {{ old('type') == 'true_false' ? 'selected' : '' }}>Verdadeiro ou
                                Falso</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="visibility_scope" name="visibility_scope"
                            class="block w-full px-4 py-3 bg-background-light border-2 border-duo-border rounded-xl text-gray-800 focus:outline-none focus:border-primary focus:ring-0 transition-all font-medium">
                            <option value="private" {{ old('visibility_scope') == 'private' ? 'selected' : '' }}>Privada
                                (Só eu)</option>
                            <option value="org_public" {{ old('visibility_scope') == 'org_public' ? 'selected' : '' }}>
                                Pública na Instituição</option>
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="level" name="level"
                            ...
                            <option value="very_easy" {{ old('level') == 'very_easy' ? 'selected' : '' }}>Muito Fácil
                            </option>
                            <option value="easy" {{ old('level') == 'easy' ? 'selected' : '' }}>Fácil</option>
                            <option value="medium" {{ old('level') == 'medium' ? 'selected' : '' }}>Médio</option>
                            <option value="hard" {{ old('level') == 'hard' ? 'selected' : '' }}>Difícil</option>
                            <option value="very_hard" {{ old('level') == 'very_hard' ? 'selected' : '' }}>Muito Difícil
                            </option>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="grade" name="grade" x-model="grade" :disabled="!stage" required
                            ...
                            <template x-for="item in availableGrades" :key="item.v">
                                <option :value="item.v" x-text="item.l"></option>
                            </template>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="space-y-2" x-data="{
                        ...
                        <select id="knowledge_area_id" name="knowledge_area_id"
                            ...
                            @foreach($knowledgeAreas as $ka)
                                <option value="{{ $ka->id }}" {{ old('knowledge_area_id') == $ka->id ? 'selected' : '' }}>
                                    {{ $ka->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="flex items-start gap-4 alternative-row">
                            ...
                            <div class="flex-1">
                                ...
                                <textarea id="question-option-{{ $i }}" name="options[]" rows="2"
                                    ...
                                    placeholder="Alternativa {{ chr(65 + $i) }}">{{ old('options.' . $i) }}</textarea>
                            </div>
                        </div>
                    @endfor
                </div>
                ...
                <div id="true_false_section" style="display: none;" role="group" aria-labelledby="true-false-heading"
                    ...
                    <label class="flex items-center gap-4 p-4 rounded-2xl border-2 border-duo-border bg-white cursor-pointer hover:border-primary/50 transition-all tf-option">
                        <input type="radio" name="tf_answer" value="0" {{ old('tf_answer', '0') == '0' ? 'checked' : '' }}
                            class="size-6 text-primary focus:ring-primary border-gray-300">
                        <div class="flex items-center gap-3">
                    ...
                    <label class="flex items-center gap-4 p-4 rounded-2xl border-2 border-duo-border bg-white cursor-pointer hover:border-primary/50 transition-all tf-option">
                        <input type="radio" name="tf_answer" value="1" {{ old('tf_answer') == '1' ? 'checked' : '' }}
                            class="size-6 text-primary focus:ring-primary border-gray-300">
                        <div class="flex items-center gap-3">
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function updatePreview() {
                ...
                if (!statementText && qType !== 'true_false' && qType !== 'essay') {
                    // Se não tiver texto nenhum e achar que deve placeholder
                ...
                if (qType === 'multiple_choice') {
                    ...
                    optionInputs.forEach((input, index) => {
                        ...
                        if (optText) {
                            ...
                            // Visual style for the option (Duolingo-like structure based on previous designs)
                            const optHtml = `
